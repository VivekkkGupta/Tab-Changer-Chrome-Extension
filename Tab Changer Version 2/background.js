...
async function getDataFromSync() {
  const synceddata = new Promise((resolve, reject) => {
...
async function getNextTabLink() {
  ...
  try {
    ...
    console.log("From getNextTabLink() Current tab : " + currentTab)
    let found = false;
...
chrome.runtime.onMessage.addListener(async (message, sender, sendResponse) => {
  if (message.action === "loadTabs") {
    ...
    // Check if the current tab is in view
    ...
    console.log("Value from getcurrenttab function : " + await getCurrentTab())

    // After switching, reload the newly switched tab
    const currentTabLink = await getCurrentTab(); // The link of the newly switched tab

    console.log("Value from nextLink : "+ currentTabLink)
